function varargout = kjm_DBS_reslice(varargin)
% KJM_DBS_RESLICE MATLAB code for kjm_DBS_reslice.fig
%      KJM_DBS_RESLICE, by itself, creates a new KJM_DBS_RESLICE or raises the existing
%      singleton*.
%
%      H = KJM_DBS_RESLICE returns the handle to a new KJM_DBS_RESLICE or the handle to
%      the existing singleton*.
%
%      KJM_DBS_RESLICE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KJM_DBS_RESLICE.M with the given input arguments.
%
%      KJM_DBS_RESLICE('Property','Value',...) creates a new KJM_DBS_RESLICE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before kjm_DBS_reslice_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to kjm_DBS_reslice_OpeningFcn via varargin.
% 
...
% Last Modified by GUIDE v2.5 23-Jan-2020 18:08:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @kjm_DBS_reslice_OpeningFcn, ...
                   'gui_OutputFcn',  @kjm_DBS_reslice_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before kjm_DBS_reslice is made visible.
function kjm_DBS_reslice_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to kjm_DBS_reslice (see VARARGIN)

% Choose default command line output for kjm_DBS_reslice
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes kjm_DBS_reslice wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = kjm_DBS_reslice_OutputFcn(hObject, eventdata, handles) 
...
function ImportMRI_Callback(hObject, eventdata, handles) % --- Executes on button press in ImportMRI.
    ...
    %% threshold and clip sides of brain image 
    
    brain_vol(brain_vol<.05)=0;

    handles.bx=find(squeeze(sum(sum(brain_vol,2),3))>0); 
    handles.by=find(squeeze(sum(sum(brain_vol,1),3))>0); 
    handles.bz=find(squeeze(sum(sum(brain_vol,1),2))>0); 

    handles.data.brain_info.mat(1:3,4)=handles.data.brain_info.mat(1:3,4)+[handles.bx(1); handles.by(1); handles.bz(1)];

    %% assign brain image
    handles.data.brain_vol=brain_vol(handles.bx,handles.by,handles.bz);
