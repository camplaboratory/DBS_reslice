...
%% calculate points for new space -- brute force approach winds up being much simpler
    ...
    poi_vol(floor(acpc_mcp(1)),floor(acpc_mcp(2)),floor(acpc_mcp(3)))=3; %MCP=3 - note, may want to recalc after rotation bc of floor command whereas ac and pc are actual pixel
...
%% rotations
    ...
    poi_vol_cubes=smooth3(poi_vol,'box',3*[1 1 1]); 
...
%% clip rotated matrices - need to insert button in program to turn this on and off
    ...
    cx=sum(brain_vol_rot,3); cx=smooth(squeeze(sum(cx,2)));
    cy=sum(brain_vol_rot,3); cy=smooth(squeeze(sum(cy,1)));
    cz=sum(brain_vol_rot,2); cz=smooth(squeeze(sum(cz,1)));
